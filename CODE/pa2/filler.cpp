/*
*  File:        filler.cpp
*  Description: Implementation of functions in the filler nameseedace.
*
*/
...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
  ...
  *        2.    if it is an appropriate frame, send the pixel PNG to the
  ...
  *        ** MINIMUM COLOR DISTANCE FROM THE pixel PIXEL **
  ...
  */
  ...
  config.neighbourorder.SetReferenceColor(seed.color);
  ...
  *config.img.getPixel(seed.x, seed.y) = config.picker->operator()(seed);
  os.Add(seed);
  ...
  while (!os.IsEmpty()) {
    ...
    for (vector<unsigned int> coord: neighbors) {
      
      bool tolerance = seed.color.dist(*config.img.getPixel(coord[0], coord[1])) <= config.tolerance;
      bool coords = coord[0]<config.img.width() && coord[0] >= 0 && coord[1] >= 0 && coord[1] < config.img.height();


      if (coords && !processed[coord[0]][coord[1]] && tolerance) {
        PixelPoint pixelPoint = PixelPoint(coord[0], coord[1], *config.img.getPixel(coord[0], coord[1]));
        config.neighbourorder.Insert(pixelPoint);
      }
  }

  while (!config.neighbourorder.IsEmpty()) {

    PixelPoint pixelPoint = config.neighbourorder.Remove();
    processed[pixelPoint.x][pixelPoint.y] = true;
    os.Add(pixelPoint);
    *config.img.getPixel(pixelPoint.x, pixelPoint.y) = config.picker->operator()(pixelPoint);
    framecount++;
